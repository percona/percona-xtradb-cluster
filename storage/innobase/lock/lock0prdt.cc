...
#ifdef WITH_WSREP
	return(rec_lock.create(NULL, trx, caller_owns_trx_mutex, true, prdt));
#else
...
#ifdef WITH_WSREP
	lock_t*	const wait_for = (lock_t *)lock_prdt_other_has_conflicting(
#else
...
#ifdef WITH_WSREP
		lock = rec_lock.create(NULL, trx, false, true);
#else
...
#endif
#ifdef WITH_WSREP
				wait_for = (lock_t *)lock_prdt_other_has_conflicting(
#else
