					...
					transaction mutex */
{
	...
	rec_lock.create(trx, caller_owns_trx_mutex, true, NULL, NULL, NULL);
#else
	RecLock		rec_lock(index, block, heap_no, type_mode);
	...
	que_thr_t*		thr)	/*!< in: query thread */
{
	...
	if (lock == NULL) {
		...
		if (!impl) {
			...
			rec_lock.create(trx, false, true, NULL, NULL, thr);
#else
			RecLock	rec_lock(index, block, heap_no, mode);
	...
	trx_t*		trx)	/*!< in: trx */
{
	...
	if(wsrep_on(trx->mysql_thd)) {
		if (c_lock && wsrep_thd_is_BF(trx->mysql_thd, FALSE)) {
        		ut_list_insert2(
				table->locks, c_lock, lock, TableLockGetNode());
			if (wsrep_debug)
	...
	que_thr_t*	thr)	/*!< in: query thread */
{
	...
	if (wait_for != NULL) {
#ifdef WITH_WSREP
		err = lock_table_enqueue_waiting(
			(lock_t *)wait_for, mode | flags, table, thr);
#else
...
#ifdef WITH_WSREP
		lock_table_create(
			(lock_t *) wait_for, table, mode | flags, trx);
#else
