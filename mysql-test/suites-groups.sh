...
function set_suites() {
  if [[ "$1" == "RelWithDebInfo" ]]; then
    ...
    # Unit tests will be executed by worker 1
    WORKER_1_MTR_SUITES="innodb_undo,test_services,audit_null,service_sys_var_registration,connection_control,service_udf_registration,service_status_var_registration,procfs,interactive_utilities,percona-pam-for-mysql,binlog,percona_binlog,innodb_zip,x,sys_vars,innodb_fts,stress"
    ...
    WORKER_4_MTR_SUITES="main|big,rpl|big,percona_rpl"
    WORKER_5_MTR_SUITES="rpl_nogtid,rpl_gtid,percona_rpl_gtid,galera|big,group_replication|nobig"
