...
function set_suites() {
  if [[ "$1" == "RelWithDebInfo" ]]; then
    ...
    WORKER_3_MTR_SUITES="engines/funcs,innodb,percona_innodb,galera_sr"
    WORKER_4_MTR_SUITES="main,rpl,percona_rpl"
    WORKER_5_MTR_SUITES="rpl_nogtid,rpl_gtid,galera|big,percona_rpl_gtid"
    ...
    WORKER_2_MTR_SUITES="galera_nbo,galera_3nodes,galera_sr,galera_3nodes_nbo,galera_3nodes_sr,galera_encryption,wsrep,galera-x,rpl|nobig,percona_rpl|nobig"
    WORKER_3_MTR_SUITES="engines/funcs,innodb,perfschema,percona_innodb,component_keyring_file"
    WORKER_4_MTR_SUITES="main|big,rpl|big,percona_rpl|big,rpl_nogtid"
    WORKER_5_MTR_SUITES="rpl_gtid,galera|big,group_replication|nobig,percona_rpl_gtid"
    WORKER_6_MTR_SUITES="parts,group_replication|big,clone|nobig,innodb_gis"
    WORKER_7_MTR_SUITES="clone|big,gcol,engines/iuds,encryption,federated,funcs_1,auth_sec,binlog_nogtid,binlog_gtid,funcs_2,jp,information_schema,rpl_encryption,sysschema,json,opt_trace,collations,gis,query_rewrite_plugins,test_service_sql_api,secondary_engine,component_audit_log_filter,component_encryption_udf,percona,component_masking_functions,component_percona_telemetry"
    WORKER_8_MTR_SUITES="galera|nobig,main|nobig,component_connection_control,jdv,component_js_lang,percona_binlog"
  fi
}
