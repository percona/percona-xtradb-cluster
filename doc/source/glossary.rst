...
.. glossary::
   ...
   MyISAM
     ...
     :term:`InnoDB`. Each table is stored on disk in 3 files: :term:`.frm`,
     :file:`.MYD`, :file:`.MYI`.
   ...
   SST
     ...
     Cluster: :program:`mysqldump`, :program:`rsync` and :program:`xtrabackup`.
     ...
     *READ-ONLY* while data is being copied from one node to another (SST
     applies :command:`FLUSH TABLES WITH READ LOCK` command). Xtrabackup SST
     does not require :command:`READ LOCK` for the entire syncing process, only
     ...
     :variable:`wsrep_sst_method` variable.
   ...
   XtraDB
     *Percona XtraDB* is an enhanced version of the InnoDB storage engine,
     ...
     <http://www.percona.com/doc/percona-server/5.6/percona_xtradb.html>`_.
   ...
   datadir
    ...
    Linux distribution use :file:`/var/lib/mysql` by default.
   ...
   quorum
     A majority (> 50%) of nodes. In the event of a network partition, only
     the cluster partition that retains a quorum (if any) will remain Primary
     by default.
