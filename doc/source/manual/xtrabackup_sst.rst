...
* Set ``encrypt=3`` to perform SST encryption based on SSL for just the key
  ...
  .. note:: The ``encrypt=3`` option can only be used
...
Used to sepersede IST on joiner node.
Requires manual setup and is not supported currently.
...
This group of options can be used to pass innobackupex options
...
For that reason, comsider the following scenarios:
