...
{
  assert(0);
  std::string error_message= message_prefix +
                             " due to internal "
                            ...
                            void *save, st_mysql_value *value)
{
  ...
  int                                         len= sizeof(buff);
  boost::movelib::unique_ptr<IKeys_container> new_keys(
...
static void update_keyring_vault_timeout(MYSQL_THD thd, st_mysql_sys_var *var,
...
static struct st_mysql_sys_var *keyring_vault_system_variables[]= {
    MYSQL_SYSVAR(config), MYSQL_SYSVAR(timeout), NULL};
                           ...
                           const char *user_id, size_t key_len)
{
  try
  {
    boost::movelib::unique_ptr<IKey> key_candidate(
