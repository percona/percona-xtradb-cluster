...
enum wsrep_trx_status wsrep_replicate(THD *thd)
{
  ...
  else if (!rcode)
  {
    if (WSREP_OK == rcode)
    {
      THD_STAGE_INFO_GUARD(NULL, NULL);
      if (WSREP(thd))
        THD_STAGE_INFO_GUARD_ENTER(thd, &stage_wsrep_replicate);
      ...
      if (WSREP(thd))
      {
        THD_STAGE_INFO_GUARD_LEAVE();
        DBUG_EXECUTE_IF("thd_proc_info.wsrep_run_wsrep_commit",
                        { thd_proc_info(thd, "wsrep: in replication stage"); };);
      }
    }
  ...
  case 0:
    ...
    */
    ...
    DBUG_PRINT("wsrep", ("replicating commit successful"));
    break;
  case WSREP_BF_ABORT:
    DBUG_ASSERT(thd->wsrep_trx_meta.gtid.seqno != WSREP_SEQNO_UNDEFINED);
  case WSREP_TRX_FAIL:
    WSREP_DEBUG("replicate failed for reason: %d %u %s",
                rcode, thd->thread_id(), WSREP_QUERY(thd));
    DBUG_PRINT("wsrep", ("replicating commit fail"));
