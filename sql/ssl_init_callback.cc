...
#ifdef WITH_WSREP
bool Ssl_init_callback_server_main::wsrep_ssl_artifacts_check(bool bootstrapping_node) {
  if (!WSREP_ON) {
...
void Ssl_init_callback_server_main::populate_wsrep_ssl_options(char *buff,
                                                    unsigned int sz) {
  memset(buff, 0, sz);
