...
void wsrep_client_rollback(THD *thd) {
  WSREP_DEBUG(
      "Initiating rollback. Transaction (query: %s) triggered by"
  ...
  if (thd->locked_tables_mode && thd->lock) {
    WSREP_DEBUG(
        "Unlock tables, transaction triggered by thread id (%u)"
        " was BF aborted and rolled back",
        thd->thread_id());
  ...
  if (thd->global_read_lock.is_acquired()) {
    WSREP_DEBUG(
        "Release Global Read Lock, transaction triggered by thread id (%u)"
        " was BF aborted and rolled back",
        thd->thread_id());
  ...
  if (thd->get_binlog_table_maps()) {
    WSREP_DEBUG(
        "Clear binlog table map, transaction triggered by thread id (%u)"
        " was BF aborted and rolled back",
        thd->thread_id());
...
static void wsrep_prepare_bf_thd(THD *thd, struct wsrep_thd_shadow *shadow) {
  ...
  */
  ...
  shadow->tx_isolation = thd->variables.transaction_isolation;
...
static void wsrep_return_from_bf_mode(THD *thd,
  ...
  thd->variables.transaction_isolation = shadow->tx_isolation;
...
void wsrep_replay_sp_transaction(THD *thd) {
  ...
  if (thd->locked_tables_mode && thd->lock) {
    WSREP_DEBUG("releasing table lock for replaying (%u)", thd->thread_id());
    thd->locked_tables_list.unlock_locked_tables(thd);
    thd->variables.option_bits &= ~(OPTION_TABLE_LOCK);
  }
  thd->mdl_context.release_transactional_locks();
  ...
  wsrep_status_t rcode = wsrep->replay_trx(wsrep, &replay_thd->wsrep_ws_handle,
                                           (void *)replay_thd);
  ...
  switch (rcode) {
    ...
    default:
      WSREP_ERROR("trx_replay failed for: %d, schema: %s, query: %s", rcode,
  ...
  wsrep_replaying--;
  WSREP_DEBUG("replaying decreased: %d, thd: %u", wsrep_replaying,
              thd->thread_id());
  ...
  if (thd->wsrep_conflict_state == MUST_REPLAY) {
    ...
    if (thd->wsrep_exec_mode != REPL_RECV) {
      if (thd->get_stmt_da()->is_sent()) {
        WSREP_ERROR(
            "Transaction replay issue, thd has reported status already");
      }
      ...
      if (thd->get_reprepare_observer() && wsrep_log_conflicts) {
        WSREP_WARN(
            "Found dangling observer in replaying of transaction"
            " (thr %u %lld %s)",
      ...
      if (da_status.status == Diagnostics_area::DA_OK) {
        ...
        strmake(da_status.message, thd->get_stmt_da()->message_text(),
      ...
      if (thd->locked_tables_mode && thd->lock) {
        WSREP_DEBUG(
            "Release table locks owned by thread (%u)."
      ...
      */
      ...
      WSREP_DEBUG(
          "Replaying transaction for SQL query: %s with write-set: %lld",
      ...
      int rcode = wsrep->replay_trx(wsrep, &thd->wsrep_ws_handle, (void *)thd);
      ...
      switch (rcode) {
        case WSREP_OK:
          ...
          if (thd->get_stmt_da()->is_sent()) {
            ...
            if (da_status.status == Diagnostics_area::DA_OK) {
              my_ok(thd, da_status.affected_rows, da_status.last_insert_id,
                    da_status.message);
            } else {
              my_ok(thd);
            }
        ...
        case WSREP_TRX_FAIL:
          if (thd->get_stmt_da()->is_sent()) {
            ...
            WSREP_DEBUG(
                "Transaction replay with write-set: %lld failed."
                " Rolling back",
                (long long)wsrep_thd_trx_seqno(thd));
        ...
        default:
          WSREP_ERROR(
              "Failed to replay transaction for: %d, schema: %s, query: %s",
              rcode, (thd->db().str ? thd->db().str : "(null)"),
              WSREP_QUERY(thd));
          /* we're now in inconsistent state, must abort */
          mysql_mutex_unlock(&thd->LOCK_wsrep_thd);
          unireg_abort(1);
      ...
      mysql_mutex_lock(&LOCK_wsrep_replaying);
      wsrep_replaying--;
      WSREP_DEBUG("Decreasing replay counter: %d, thd: %u", wsrep_replaying,
                  thd->thread_id());
      mysql_cond_broadcast(&COND_wsrep_replaying);
      mysql_mutex_unlock(&LOCK_wsrep_replaying);
    }
...
static bool create_wsrep_THD(PSI_thread_key key,
...
static void wsrep_rollback_process(THD *thd) {
  ...
  while (thd->killed == THD::NOT_KILLED) {
    ...
    WSREP_DEBUG(
        "Rollback/Aborter thread wakes for signal."
    ...
    while (wsrep_aborting_thd) {
      ...
      if (aborting->wsrep_conflict_state == ABORTED) {
        WSREP_DEBUG(
            "Rollback/Aborter encounter already aborted thread"
      ...
      WSREP_DEBUG(
          "Rollback/Aborter thread successfully aborted thread"
          " thread-id: %u (%llu) conflict-state: %s",
          aborting->thread_id(), (long long)aborting->real_id,
          wsrep_get_conflict_state(aborting->wsrep_conflict_state));
      mysql_mutex_unlock(&aborting->LOCK_wsrep_thd);
...
void wsrep_thd_set_PA_safe(void *thd_ptr, bool safe) {
  if (thd_ptr) {
    THD *thd = (THD *)thd_ptr;
...
extern "C" bool wsrep_thd_is_BF_or_commit(void *thd_ptr, bool sync) {
  bool status = false;
  if (thd_ptr) {
    THD *thd = (THD *)thd_ptr;
...
extern "C" bool wsrep_thd_is_local(void *thd_ptr, bool sync) {
  bool status = false;
  if (thd_ptr) {
    THD *thd = (THD *)thd_ptr;
...
int wsrep_abort_thd(void *bf_thd_ptr, void *victim_thd_ptr, bool signal) {
...
extern "C" void wsrep_thd_auto_increment_variables(
    THD *thd, unsigned long long *offset, unsigned long long *increment) {
  if (thd->wsrep_exec_mode == REPL_RECV &&
      thd->wsrep_conflict_state != REPLAYING) {
